...
class UserDataUpdateEventDeserializer : JsonDeserializer<UserDataEvent>() {
    ...
    override fun deserialize(jp: JsonParser, ctx: DeserializationContext): UserDataEvent {
        ...
        return when (eventTypeId) {
            "outboundAccountInfo", "outboundAccountPosition" -> JsonToObject.convert(json, UserDataEvent.AccountUpdateEvent::class.java)
            "balanceUpdate" -> JsonToObject.convert(json, UserDataEvent.BalanceUpdateEvent::class.java)
            "executionReport" -> JsonToObject.convert(json, UserDataEvent.OrderTradeUpdateEvent::class.java)
